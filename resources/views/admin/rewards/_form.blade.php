...
<div class="row g-3">
    ...
    <div class="col-md-4 mb-3">
        <label class="form-label" for="moneyInput">{{ trans('messages.fields.money') }} bonus</label>

        <div class="input-group @error('money_bonus') has-validation @enderror">
            <input type="number" min="0" step="0.01" max="999999" class="form-control @error('money_bonus') is-invalid @enderror" id="moneyInput" name="money_bonus" value="{{ old('money_bonus', $reward->money_bonus ?? '') }}">
            <div class="input-group-text">{{ money_name() }}</div>
...
{{--        <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>--}}
    ...
    <div class="mb-3 col-md-6">
        <label class="form-label" for="image_bonus">{{ trans('messages.fields.image') }} Bonus</label>
        <input type="file" class="form-control @error('image_bonus') is-invalid @enderror" id="image_bonus" name="image_bonus" accept=".jpg,.jpeg,.jpe,.png,.gif,.bmp,.svg,.webp" data-image-preview="image_bonus">
...
<div class="mb-3">
    <label for="commands_bonus" class="form-label">Commande pour rôle supplémentaires</label>
    @include('admin.elements.list-input', ['name' => 'commands_bonus', 'values' => $reward->commands_bonus ?? []])
</div>
...
<hr>
<div class="mb-3 form-check form-switch">
    <input type="checkbox" class="form-check-input" id="enableSwitch_Accept" name="double_accept" @checked(old('double_accept', $reward->double_accept ?? false))>
    <label class="form-check-label" for="enableSwitch">Rendre cette récompense en doublé autorisé</label>
</div>

<div class="mb-3">
    <label class="form-label">Sélectionner les rôles bonus </label>
    @foreach($roles as $role)
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="roles_authorized[]" id="role_{{ $role->id }}" value="{{ $role->id }}" @if($reward->roles_authorized && in_array($role->id, $reward->roles_authorized)) checked @endif>
